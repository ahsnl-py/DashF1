...
--
-- Name: udf_driver_stats(integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.udf_driver_stats(year integer) RETURNS TABLE(driver_id smallint, driver_fullname character varying, driver_team character varying, total_first_place smallint, total_second_place smallint, total_third_place smallint, total_points smallint, average_point_gp numeric, total_count_podiums smallint, win_ration numeric, count_fastest_lap smallint, total_lap_raced integer, total_lap_not_raced integer)
    LANGUAGE sql
    AS $_$
 	SELECT 	driver_id
			, driver_fullname
			, max(constructors_name) as driver_team
			, sum(iswin) total_win
			...
			, sum(fastest_lap)
			, sum(laps) 
			, sum(laps_not_completed)
	FROM (
		select *
				, case when status_id = 1 then 0
					else case when status_id <> 1 then max(laps) over(partition by gp_name) - laps
						end 
					end laps_not_completed
				, case when (status_id = 1 and position = 1) then max(laps) over(partition by gp_name) 
					end lap_per_gp
...
CREATE VIEW public.vw_race_results AS
 SELECT fc.constructor_id,
    ...
    fsrs.fastest_lap_time,
        CASE
            WHEN (fsrs.rank = 1) THEN 1
            ELSE 0
        END AS fastest_lap
